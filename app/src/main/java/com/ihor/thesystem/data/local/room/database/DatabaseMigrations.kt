...
object DatabaseMigrations {
    ...
    val MIGRATION_37_38 = object : Migration(37, 38) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL("ALTER TABLE system_config ADD COLUMN lastInitEpochDay INTEGER NOT NULL DEFAULT 0")
        }
    }
    ...
    val MIGRATION_38_39 = object : Migration(38, 39) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL(
                "CREATE INDEX IF NOT EXISTS `index_schedule_task_cross_ref_taskTemplateId` " +
                    "ON `schedule_task_cross_ref` (`taskTemplateId`)"
            )
        }
    }
    ...
    val MIGRATION_40_41 = object : Migration(40, 41) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL(
                """
                CREATE TABLE IF NOT EXISTS `calendar_cycle_config` (
                    `id` INTEGER NOT NULL,
                    `name` TEXT NOT NULL,
            ...
            db.execSQL(
                """
                CREATE TABLE IF NOT EXISTS `calendar_cycle_day` (
                    `cycleId` INTEGER NOT NULL,
                    `dayIndex` INTEGER NOT NULL,
                    `name` TEXT NOT NULL,
                    `type` TEXT NOT NULL,
                    PRIMARY KEY(`cycleId`, `dayIndex`),
                    FOREIGN KEY(`cycleId`) REFERENCES `calendar_cycle_config`(`id`)
                        ON UPDATE NO ACTION ON DELETE CASCADE
                )
                """.trimIndent()
            )
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_calendar_cycle_day_cycleId` ON `calendar_cycle_day` (`cycleId`)")
        }
    }
    ...
    val MIGRATION_41_42 = object : Migration(41, 42) {
        override fun migrate(db: SupportSQLiteDatabase) {
            ...
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_todo_dateEpochDay` ON `todo` (`dateEpochDay`)")
        }
    }
    ...
    val MIGRATION_45_46 = object : Migration(45, 46) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL("DROP INDEX IF EXISTS `index_schedule_cycleDay`")
            db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS `index_schedule_cycleDay` ON `schedule` (`cycleDay`)")
        }
    }
    ...
    val MIGRATION_46_47 = object : Migration(46, 47) {
        override fun migrate(db: SupportSQLiteDatabase) {
            ...
            db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS `index_readiness_entries_dateEpochDay` ON `readiness_entries` (`dateEpochDay`)")
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_readiness_entries_createdAtMillis` ON `readiness_entries` (`createdAtMillis`)")
        }
    }
        ...
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL(
                ...
                CREATE TABLE IF NOT EXISTS `equipment_profile` (
                    `id` INTEGER NOT NULL,
                    `trainsAtGym` INTEGER NOT NULL DEFAULT 0,
                    `availableEquipment` TEXT NOT NULL DEFAULT 'BODY_ONLY',
                    `dumbbellMaxKg` REAL,
                    `barbellAvailable` INTEGER NOT NULL DEFAULT 0,
                    `benchAvailable` INTEGER NOT NULL DEFAULT 0,
                    `pullUpBarAvailable` INTEGER NOT NULL DEFAULT 0,
                    `dipBarsAvailable` INTEGER NOT NULL DEFAULT 0,
                    `bandsAvailable` INTEGER NOT NULL DEFAULT 0,
                    `machinesAvailable` INTEGER NOT NULL DEFAULT 0,
                    PRIMARY KEY(`id`)
                )
                """.trimIndent()
            )
    ...
    val MIGRATION_48_49 = object : Migration(48, 49) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL("ALTER TABLE `exercises` ADD COLUMN `isCoreSystemExercise` INTEGER NOT NULL DEFAULT 0")
            db.execSQL("ALTER TABLE `exercises` ADD COLUMN `movementPattern` TEXT")
            db.execSQL("ALTER TABLE `exercises` ADD COLUMN `techniqueTips` TEXT NOT NULL DEFAULT '[]'")
            db.execSQL("ALTER TABLE `exercises` ADD COLUMN `commonMistakes` TEXT NOT NULL DEFAULT '[]'")
            db.execSQL("ALTER TABLE `exercises` ADD COLUMN `substitutionExternalIds` TEXT NOT NULL DEFAULT '[]'")
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_exercises_isCoreSystemExercise` ON `exercises` (`isCoreSystemExercise`)")
        }
    }
        ...
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL(
                ...
                CREATE TABLE IF NOT EXISTS `nutrition_entries` (
                    ...
                    `waterHit` INTEGER NOT NULL,
                    `mealsQuality` TEXT NOT NULL,
                    `bodyWeight` REAL,
            ...
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_nutrition_entries_dateEpochDay` ON `nutrition_entries` (`dateEpochDay`)")
        }
    }
    ...
    val MIGRATION_50_51 = object : Migration(50, 51) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL(
                "ALTER TABLE `exercises` ADD COLUMN `coreMetadataVersion` INTEGER NOT NULL DEFAULT 0"
            )
        }
    }
