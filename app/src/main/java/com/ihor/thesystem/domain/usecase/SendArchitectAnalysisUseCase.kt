...
class SendArchitectAnalysisUseCase @Inject constructor(
    ...
    suspend operator fun invoke(workoutContext: String): ChatMessage {
        val prompt = """
            ...
            4) Запропонувати вагу та повторення на наступне тренування.
            
            ПРОТОКОЛ ПРОГРЕСІЇ:
            - Якщо користувач виконав цільову кількість повторень у всіх сетах (успіх) — збільш вагу на 2.5-5% для наступного тренування.
            - Якщо користувач НЕ виконав цільову кількість повторень хоча б в одному сеті (failure) — запропонуй деструктивний протокол (deload): зменш цільову вагу на 10% для наступного тренування.
